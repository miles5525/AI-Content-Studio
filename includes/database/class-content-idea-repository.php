...
final class AICS_Content_Idea_Repository {
	...
	public function get_next_article_idea_for_run( $run_id, $profile_id ): ?array { global $wpdb;$run=absint($run_id);$profile=absint($profile_id);if(0===$run||0===$profile){return null;}$sql=$wpdb->prepare("SELECT * FROM {$this->table()} WHERE run_id=%d AND profile_id=%d AND source_type='automation' AND status IN ('queued','article_generating') ORDER BY priority DESC,score DESC,CASE WHEN planned_publish_at IS NULL THEN 1 ELSE 0 END ASC,planned_publish_at ASC,created_at ASC,id ASC LIMIT 1",$run,$profile);return $this->normalize_row($wpdb->get_row($sql,ARRAY_A));}
