...
final class AICS_Content_Idea_Repository {
	...
	public function count_for_run_ids(array $run_ids):array{global $wpdb;$ids=array_values(array_unique(array_filter(array_map('absint',$run_ids))));if(!$ids){return array();}$marks=implode(',',array_fill(0,count($ids),'%d'));$sql=$wpdb->prepare("SELECT run_id,COUNT(*) total FROM {$this->table()} WHERE run_id IN ({$marks}) GROUP BY run_id",$ids);$out=array();foreach($wpdb->get_results($sql,ARRAY_A) as $row){$out[absint($row['run_id'])]=absint($row['total']);}return $out;}
	/** Returns one deterministic queued/recoverable article idea for a run and profile. */
	public function get_next_article_idea_for_run( $run_id, $profile_id ): ?array { global $wpdb;$run=absint($run_id);$profile=absint($profile_id);if(0===$run||0===$profile){return null;}$sql=$wpdb->prepare("SELECT i.* FROM {$this->table()} i LEFT JOIN {$wpdb->prefix}aics_articles a ON a.idea_id=i.id LEFT JOIN {$wpdb->prefix}aics_automation_runs r ON r.id=i.run_id WHERE i.run_id=%d AND i.profile_id=%d AND i.source_type='automation' AND i.status IN ('queued','article_generating') ORDER BY CASE WHEN i.status='article_generating' AND a.status='queued' AND a.title='' AND a.excerpt='' AND a.content='' AND a.generation_attempts<r.max_attempts THEN 0 WHEN i.status='article_generating' AND a.status='generating' AND a.title='' AND a.excerpt='' AND a.content='' AND a.generation_attempts<r.max_attempts THEN 1 ELSE 2 END ASC,i.priority DESC,i.score DESC,CASE WHEN i.planned_publish_at IS NULL THEN 1 ELSE 0 END ASC,i.planned_publish_at ASC,i.created_at ASC,i.id ASC LIMIT 1",$run,$profile);return $this->normalize_row($wpdb->get_row($sql,ARRAY_A));}
	...
	public function store_evaluation($idea_id,$score,$updated_by=0):array{global $wpdb;$id=absint($idea_id);$score=self::decimal($score);$user=self::nonnegative($updated_by);if(null===$score){return self::simple(false,$id,'invalid_idea_score');}if(null===$user){return self::simple(false,$id,'invalid_updated_by');}$idea=$this->get_by_id($id);if(!$idea){return self::simple(false,0,'idea_not_found');}if('generated'!==$idea['status']){return self::simple(false,$id,'idea_not_evaluable');}if(null!==$idea['evaluated_at']){return self::simple(false,$id,'idea_already_evaluated');}$now=self::now();$sql=$wpdb->prepare("UPDATE {$this->table()} SET score=%s,evaluated_at=%s,updated_by=%d,updated_at=%s WHERE id=%d AND status='generated' AND evaluated_at IS NULL",number_format($score,2,'.',''),$now,$user,$now,$id);$changed=$wpdb->query($sql);return 1===$changed?self::simple(true,$id,'idea_evaluation_stored'):self::simple(false,$id,'idea_already_evaluated');}
	public function get_unevaluated_ideas_for_run($run_id,$limit=20):array{return $this->get_ideas(array('run_id'=>absint($run_id),'source_type'=>'automation','status'=>'generated','evaluated'=>'no','limit'=>max(1,min(20,absint($limit))),'orderby'=>'created_at','order'=>'ASC'));}
	public function get_evaluated_ideas_for_run($run_id,array $args=array()):array{$args['run_id']=absint($run_id);$args['source_type']='automation';$args['evaluated']='yes';$args['limit']=max(1,min(20,absint($args['limit']??20)));return $this->get_ideas($args);}
	public function count_ideas_for_run_by_status($run_id):array{global $wpdb;$id=absint($run_id);if(0===$id){return array();}$sql=$wpdb->prepare("SELECT status,COUNT(*) total FROM {$this->table()} WHERE run_id=%d GROUP BY status",$id);$out=array();foreach($wpdb->get_results($sql,ARRAY_A) as $row){$status=sanitize_key($row['status']??'');if(in_array($status,self::STATUSES,true)){$out[$status]=absint($row['total']??0);}}return $out;}
	...
	private function filters(array $args):array{$where=array('1=1');$values=array();foreach(array('profile_id','run_id') as $f){if(isset($args[$f])){$v=self::nonnegative($args[$f]);if(null===$v){$where[]='1=0';}else{$where[]="{$f}=%d";$values[]=$v;}}}foreach(array('source_type'=>self::SOURCES,'status'=>self::STATUSES,'search_intent'=>self::INTENTS) as $f=>$allowed){if(isset($args[$f])){if(in_array($args[$f],$allowed,true)){$where[]="{$f}=%s";$values[]=$args[$f];}else{$where[]='1=0';}}}if(isset($args['statuses'])){$submitted=array();foreach(is_array($args['statuses'])?$args['statuses']:array() as $status){if(is_scalar($status)){$submitted[]=sanitize_key((string)$status);}}$s=array_values(array_intersect(array_unique($submitted),self::STATUSES));if($s){$where[]='status IN ('.implode(',',array_fill(0,count($s),'%s')).')';array_push($values,...$s);}else{$where[]='1=0';}}if(isset($args['evaluated'])){$where[]='yes'===$args['evaluated']?'evaluated_at IS NOT NULL':('no'===$args['evaluated']?'evaluated_at IS NULL':'1=0');}if(isset($args['content_fingerprint'])){if(is_string($args['content_fingerprint'])&&preg_match('/^[a-f0-9]{64}$/',$args['content_fingerprint'])){$where[]='content_fingerprint=%s';$values[]=$args['content_fingerprint'];}else{$where[]='1=0';}}foreach(array('planned_before'=>array('planned_publish_at','<='),'planned_after'=>array('planned_publish_at','>='),'created_before'=>array('created_at','<='),'created_after'=>array('created_at','>=')) as $key=>$spec){if(isset($args[$key])){$d=self::datetime($args[$key]);if(null===$d){$where[]='1=0';}else{$where[]=$spec[0].$spec[1].'%s';$values[]=$d;}}}return array($where,$values);}
	private function normalize_row($row):?array{if(!is_array($row)){return null;}foreach(array('id','profile_id','run_id','priority','approved_by','rejected_by','created_by','updated_by') as $f){$row[$f]=absint($row[$f]??0);}$row['score']=(float)($row['score']??0);$row['source_type']=in_array($row['source_type']??'',self::SOURCES,true)?$row['source_type']:'automation';$row['status']=in_array($row['status']??'',self::STATUSES,true)?$row['status']:'failed';$row['search_intent']=in_array($row['search_intent']??'',self::INTENTS,true)?$row['search_intent']:'informational';$row['secondary_keywords']=self::decode($row['secondary_keywords']??'');$row['outline']=self::decode($row['outline']??'');foreach(array('evaluated_at','planned_publish_at','approved_at','rejected_at') as $f){$row[$f]=self::datetime($row[$f]??null);}return $row;}
