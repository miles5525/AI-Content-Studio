...
final class AICS_Automation_Article_Generator {
	...
	private function log( int $article_id, bool $success, string $code, int $duration, string $length, string $tone ): void { AICS_Usage_Logger::log( array( 'user_id'=>0,'event_type'=>'ai_request','operation'=>'generate_article_draft','status'=>$success?'success':'failed','provider'=>'openai','model'=>AICS_Settings::get_openai_model(),'error_code'=>$code,'object_id'=>$article_id,'duration_ms'=>$duration,'metadata'=>array('requested_length'=>$length,'tone'=>$tone,'source'=>'automation') ) ); }
}
