...
final class AICS_Media_Library_Image_Service {
	...
	public function upload(AICS_Temporary_Image_File $temporary,array $article,array $generation,string $alt_text){
		...
		$metadata=array('_aics_generated_image'=>'1','_aics_image_source'=>'generated','_aics_article_id'=>(string)$article_id,'_aics_article_uuid'=>$uuid,'_aics_run_id'=>(string)absint($article['run_id']??0),'_aics_profile_id'=>(string)absint($article['profile_id']??0),'_aics_post_id'=>(string)$post_id,'_aics_provider'=>sanitize_key((string)($generation['provider']??'')),'_aics_model'=>sanitize_text_field((string)($generation['model']??'')),'_aics_prompt_hash'=>hash('sha256',$prompt));
