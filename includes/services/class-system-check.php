...
final class AICS_System_Check {
	...
	public static function diagnostics( array $checks ): string {
		...
		$lines = array( 'AI Content Studio Diagnostics', 'Plugin version: ' . AICS_VERSION, 'Schema version: ' . AICS_DB_VERSION, 'WordPress version: ' . get_bloginfo( 'version' ), 'PHP version: ' . PHP_VERSION, 'Site language: ' . get_locale(), 'Multisite: ' . ( is_multisite() ? 'yes' : 'no' ), 'HTTPS: ' . ( $map['https'] ?? 'unknown' ), 'REST API: ' . ( $map['rest_api'] ?? 'unknown' ), 'cURL: ' . ( $map['curl'] ?? 'unknown' ), 'JSON: ' . ( $map['json'] ?? 'unknown' ), 'OpenSSL: ' . ( $map['openssl'] ?? 'unknown' ), 'Uploads: ' . ( $map['uploads'] ?? 'unknown' ), 'Provider: OpenAI', 'API key configured: ' . ( AICS_Settings::has_openai_api_key() ? 'yes' : 'no' ), 'Selected model: ' . AICS_Settings::get_openai_model(), 'Usage table: ' . ( $map['usage_table'] ?? 'unknown' ), 'Cleanup cron: ' . ( false !== wp_next_scheduled( 'aics_cleanup_usage_logs' ) ? 'scheduled' : 'not scheduled' ), 'WP_DEBUG: ' . ( defined( 'WP_DEBUG' ) && WP_DEBUG ? 'enabled' : 'disabled' ), 'PHP memory limit: ' . ini_get( 'memory_limit' ), 'Maximum execution time: ' . (string) ini_get( 'max_execution_time' ) );
