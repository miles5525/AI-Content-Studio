...
final class AICS_System_Check {
	...
	public static function run(): array {
		...
		$seo_detection = (new AICS_SEO_Plugin_Detector())->resolve('auto');
		$seo_adapter = AICS_SEO_Adapter_Factory::get('auto');
		$seo_value = is_wp_error($seo_adapter)?__('Unavailable','ai-content-studio'):$seo_adapter->get_label().' '.($seo_adapter->get_version()?:'');
		$seo_fields = is_wp_error($seo_adapter)?'':implode(', ',array_filter(array($seo_adapter->supports_seo_title()?'title':'',$seo_adapter->supports_meta_description()?'description':'',$seo_adapter->supports_focus_keyword()?'focus keyword':'')));
		...
		return array(
			...
			self::item( 'seo_adapter', __( 'SEO Adapter', 'ai-content-studio' ), !empty($seo_detection['multiple'])?'warning':'good', trim($seo_value), sprintf(__('Compatibility: %1$s. Plugin fields: %2$s. Detected: %3$s.','ai-content-studio'),is_wp_error($seo_adapter)?'unavailable':$seo_adapter->get_compatibility_status(),$seo_fields?:__('native fields only','ai-content-studio'),$seo_detection['active']?implode(', ',$seo_detection['active']):__('none','ai-content-studio')) ),
			...
			self::item( 'automation_worker', __( 'Core Automation Engine', 'ai-content-studio' ), false === $worker_next || false === $automation_next || ! $runs_ok || ! $ideas_ok || ! $articles_ok || ! class_exists('AICS_Automation_Delivery_Service') ? 'critical' : ( $cron_disabled ? 'warning' : 'good' ), false === $worker_next ? __( 'Not scheduled', 'ai-content-studio' ) : wp_date( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), $worker_next, wp_timezone() ), ! $runs_ok || ! $ideas_ok || ! $articles_ok ? __( 'A required automation table is missing.', 'ai-content-studio' ) : ( false === $worker_next || false === $automation_next ? __( 'A required automation event is missing.', 'ai-content-studio' ) : ( $cron_disabled ? __( 'The events exist, but WP-Cron is disabled. A real server cron may still trigger WordPress Cron.', 'ai-content-studio' ) : __( 'The dispatcher, worker, delivery service, and required tables are available.', 'ai-content-studio' ) ) ) ),
