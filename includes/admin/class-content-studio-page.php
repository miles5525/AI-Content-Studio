...
final class AICS_Content_Studio_Page {
	...
	private const GENERATE_SEO_ACTION = 'aics_generate_manual_seo_data';
	private const SAVE_SEO_ACTION = 'aics_save_manual_seo_data';
	private const APPLY_SEO_ACTION = 'aics_apply_manual_seo_data';
	private const SEO_RESULT_PREFIX = 'aics_manual_seo_result_';
	...
	public static function register(): void {
		...
		add_action( 'admin_post_' . self::GENERATE_SEO_ACTION, array( self::class, 'handle_generate_seo' ) );
		add_action( 'admin_post_' . self::SAVE_SEO_ACTION, array( self::class, 'handle_save_seo' ) );
		add_action( 'admin_post_' . self::APPLY_SEO_ACTION, array( self::class, 'handle_apply_seo' ) );
	...
	public static function render(): void {
		...
		<div class="wrap aics-admin-wrap aics-content-studio-page">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php self::render_trusted_external_controls($state['seo_settings'],'seo_settings'); ?>
				<?php self::render_manual_seo_quality_gate($state['seo_settings']); ?>
	 ...
	 * @return void
	 */
	public static function handle_generate_seo():void{self::require_permission();$draft=self::get_article_draft();$id=absint($_POST['article_id']??0);$nonce=isset($_POST['aics_manual_seo_nonce'])&&is_string($_POST['aics_manual_seo_nonce'])?sanitize_text_field(wp_unslash($_POST['aics_manual_seo_nonce'])):'';if(!$draft||$id!==absint($draft['article_id'])||!wp_verify_nonce($nonce,self::GENERATE_SEO_ACTION.'_'.$id)){self::store_seo_result(array('success'=>false,'code'=>'seo_article_source_invalid','message'=>__('The SEO request could not be verified.','ai-content-studio')));self::redirect('manual-seo-result');}$result=(new AICS_Manual_SEO_Service())->generate($id,get_current_user_id());self::store_seo_result($result);self::redirect('manual-seo-result');}

	public static function handle_save_seo():void{self::require_permission();$draft=self::get_article_draft();$id=absint($_POST['article_id']??0);$nonce=isset($_POST['aics_manual_seo_save_nonce'])&&is_string($_POST['aics_manual_seo_save_nonce'])?sanitize_text_field(wp_unslash($_POST['aics_manual_seo_save_nonce'])):'';if(!$draft||$id!==absint($draft['article_id'])||!wp_verify_nonce($nonce,self::SAVE_SEO_ACTION.'_'.$id)){self::store_seo_result(array('success'=>false,'code'=>'seo_article_source_invalid','message'=>__('The SEO save request could not be verified.','ai-content-studio')));self::redirect('manual-seo-result');}$scalar=static fn($k):string=>isset($_POST[$k])&&is_string($_POST[$k])?wp_unslash($_POST[$k]):'';$input=array('focus_keyword'=>$scalar('seo_focus_keyword'),'seo_title'=>$scalar('seo_title'),'meta_description'=>$scalar('seo_meta_description'),'slug'=>$scalar('seo_slug'),'excerpt'=>$scalar('seo_excerpt'),'categories'=>array_map('trim',explode(',',$scalar('seo_categories'))),'tags'=>array_map('trim',explode(',',$scalar('seo_tags'))),'image_alt_text'=>$scalar('seo_image_alt_text'));$result=(new AICS_Manual_SEO_Service())->save($id,get_current_user_id(),$input);self::store_seo_result($result);self::redirect('manual-seo-result');}

	public static function handle_apply_seo():void{self::require_permission();$draft=self::get_article_draft();$id=absint($_POST['article_id']??0);$nonce=isset($_POST['aics_manual_seo_apply_nonce'])&&is_string($_POST['aics_manual_seo_apply_nonce'])?sanitize_text_field(wp_unslash($_POST['aics_manual_seo_apply_nonce'])):'';if(!$draft||$id!==absint($draft['article_id'])||!wp_verify_nonce($nonce,self::APPLY_SEO_ACTION.'_'.$id)){self::store_seo_result(array('success'=>false,'code'=>'seo_post_ownership_invalid','message'=>__('The SEO application request could not be verified.','ai-content-studio')));self::redirect('manual-seo-result');}$result=(new AICS_Manual_SEO_Application_Service())->apply($id,get_current_user_id());self::store_seo_result($result);self::redirect('manual-seo-result');}
	 ...
	 * @return array{business_context: string, topic_keyword: string, tone: string, article_length: string}
	 */
	private static function render_trusted_external_controls(array $s,string $name):void{?><section class="aics-content-section"><h3><?php esc_html_e('Trusted External Link Sources','ai-content-studio');?></h3><p><?php esc_html_e('Only public HTTP(S) URLs entered here can be verified and considered. No web search is performed.','ai-content-studio');?></p><p><label><?php esc_html_e('Trusted external URLs, one per line','ai-content-studio');?><br><textarea class="large-text" rows="4" name="<?php echo esc_attr($name);?>[trusted_external_urls]"><?php echo esc_textarea(implode("\n",$s['trusted_external_urls']??array()));?></textarea></label></p><p><label><?php esc_html_e('Trusted external domains, one per line','ai-content-studio');?><br><textarea class="large-text" rows="3" name="<?php echo esc_attr($name);?>[trusted_external_domains]"><?php echo esc_textarea(implode("\n",$s['trusted_external_domains']??array()));?></textarea></label></p></section><?php }
	private static function render_manual_seo_quality_gate(array $s):void{$q=AICS_SEO_Configuration::quality_gate($s);?><section class="aics-content-section"><h3><?php esc_html_e('SEO Quality Guidance','ai-content-studio');?></h3><p><?php esc_html_e('Manual Studio shows this threshold as guidance and never prevents saving a draft.','ai-content-studio');?></p><label><?php esc_html_e('Suggested minimum AICS SEO Readiness','ai-content-studio');?> <input type="number" min="0" max="100" name="seo_settings[quality_gate][minimum_readiness_score]" value="<?php echo esc_attr($q['minimum_readiness_score']);?>"></label><input type="hidden" name="seo_settings[quality_gate][enabled]" value="0"></section><?php }
	...
	private static function render_article_draft( ?array $validated_inputs, ?array $selected_idea, ?array $article_draft ): void {
		...
		<section class="aics-article-section" aria-labelledby="aics-article-heading">
			...
			<?php else : ?>
				...
				<?php self::render_seo_review($article_draft); ?>
				<?php self::render_seo_application($article_draft); ?>
				<?php self::render_seo_application_details($article_draft); ?>
			<?php endif; ?>
		</section>
		<?php
	}
	 ...
	 * @return bool
	 */
	private static function render_seo_review(array $draft):void{$id=absint($draft['article_id']??0);if(!$id){return;}$view=(new AICS_Manual_SEO_Service())->view($id,get_current_user_id());if(empty($view['success'])){return;}$a=$view['article'];$analysis=is_array($a['seo_analysis']??null)?$a['seo_analysis']:array();$has=''!==(string)($a['seo_focus_keyword']??'');$profile=$view['profile'];?><section class="aics-content-section aics-seo-review"><h3><?php esc_html_e('SEO Data','ai-content-studio');?></h3><p><strong><?php esc_html_e('Status:','ai-content-studio');?></strong> <?php echo esc_html(AICS_SEO_State::label($a['seo_status']));?> · <strong><?php esc_html_e('Target:','ai-content-studio');?></strong> <?php echo esc_html($profile['label']);?></p><?php if(!$has):self::render_seo_generate_form($id,__('Generate SEO Data','ai-content-studio'));else:?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>"><input type="hidden" name="action" value="<?php echo esc_attr(self::SAVE_SEO_ACTION);?>"><input type="hidden" name="article_id" value="<?php echo esc_attr((string)$id);?>"><?php wp_nonce_field(self::SAVE_SEO_ACTION.'_'.$id,'aics_manual_seo_save_nonce');?><p><label><strong><?php esc_html_e('Focus keyword','ai-content-studio');?></strong><br><input class="large-text" maxlength="150" name="seo_focus_keyword" value="<?php echo esc_attr($a['seo_focus_keyword']);?>" required></label></p><p><label><strong><?php esc_html_e('SEO title','ai-content-studio');?></strong><br><input class="large-text" maxlength="250" name="seo_title" value="<?php echo esc_attr($a['seo_title']);?>" required></label><span class="description"><?php echo esc_html(sprintf(__('%1$d characters; recommended %2$d–%3$d.','ai-content-studio'),self::text_length((string)$a['seo_title']),$profile['title_min'],$profile['title_max']));?></span></p><p><label><strong><?php esc_html_e('Meta description','ai-content-studio');?></strong><br><textarea class="large-text" maxlength="500" rows="3" name="seo_meta_description" required><?php echo esc_textarea($a['seo_meta_description']);?></textarea></label><span class="description"><?php echo esc_html(sprintf(__('%1$d characters; recommended %2$d–%3$d.','ai-content-studio'),self::text_length((string)$a['seo_meta_description']),$profile['description_min'],$profile['description_max']));?></span></p><p><label><strong><?php esc_html_e('Slug','ai-content-studio');?></strong><br><input class="large-text" maxlength="200" name="seo_slug" value="<?php echo esc_attr($a['seo_slug']);?>" required></label></p><p><label><strong><?php esc_html_e('SEO excerpt','ai-content-studio');?></strong><br><textarea class="large-text" maxlength="1000" rows="3" name="seo_excerpt"><?php echo esc_textarea($a['excerpt']);?></textarea></label></p><p><label><strong><?php esc_html_e('Suggested categories','ai-content-studio');?></strong><br><input class="large-text" name="seo_categories" value="<?php echo esc_attr(implode(', ',$a['seo_categories']));?>"></label></p><p><label><strong><?php esc_html_e('Suggested tags','ai-content-studio');?></strong><br><input class="large-text" name="seo_tags" value="<?php echo esc_attr(implode(', ',$a['seo_tags']));?>"></label></p><p><label><strong><?php esc_html_e('Suggested image alt text','ai-content-studio');?></strong><br><input class="large-text" maxlength="250" name="seo_image_alt_text" value="<?php echo esc_attr($a['featured_image_alt_text']);?>"></label></p><?php submit_button(__('Save SEO Data','ai-content-studio'),'primary','submit',false);?></form><?php if('applied'!==$a['seo_status']):self::render_seo_generate_form($id,__('Regenerate SEO Data','ai-content-studio'));endif;self::render_seo_analysis($analysis,$profile);endif;?></section><?php }
	private static function render_seo_generate_form(int $id,string $label):void{?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>" data-aics-generating-label="<?php echo esc_attr__('Generating…','ai-content-studio');?>"><input type="hidden" name="action" value="<?php echo esc_attr(self::GENERATE_SEO_ACTION);?>"><input type="hidden" name="article_id" value="<?php echo esc_attr((string)$id);?>"><?php wp_nonce_field(self::GENERATE_SEO_ACTION.'_'.$id,'aics_manual_seo_nonce');?><?php submit_button($label,'secondary','submit',false);?></form><?php }
	private static function render_seo_analysis(array $a,array $profile):void{?><div class="aics-seo-analysis"><h4><?php esc_html_e('SEO Readiness','ai-content-studio');?></h4><p><strong><?php echo esc_html((string)absint($a['score']??0).'/100 · '.sanitize_text_field((string)($a['label']??'Needs Work')));?></strong></p><p class="description"><?php echo esc_html($profile['note']);?> <?php esc_html_e('This readiness score does not guarantee rankings or a plugin score.','ai-content-studio');?></p><?php foreach(array('passed_checks'=>__('Passed checks','ai-content-studio'),'warnings'=>__('Warnings','ai-content-studio'),'blocking_issues'=>__('Blocking issues','ai-content-studio')) as $key=>$label):?><h5><?php echo esc_html($label);?></h5><ul><?php foreach(array_slice(is_array($a[$key]??null)?$a[$key]:array(),0,30) as $item):?><li><?php echo esc_html(sanitize_text_field((string)($item['message']??'')));?></li><?php endforeach;?></ul><?php endforeach;?></div><?php }
	private static function text_length(string $v):int{return function_exists('mb_strlen')?mb_strlen($v,'UTF-8'):strlen($v);}

	private static function render_seo_application(array $draft):void{$id=absint($draft['article_id']??0);if(!$id){return;}$p=(new AICS_Manual_SEO_Application_Service())->preview($id,get_current_user_id());if(empty($p['success'])||empty($p['article']['seo_focus_keyword'])){return;}$a=$p['article'];$post=get_post($a['wordpress_post_id']);?><section class="aics-content-section aics-seo-application"><h3><?php esc_html_e('Native WordPress SEO Application','ai-content-studio');?></h3><p><?php esc_html_e('Preview only. Applying updates the owned WordPress draft and does not write Yoast, Rank Math, or AIOSEO metadata.','ai-content-studio');?></p><dl><dt><?php esc_html_e('Target post','ai-content-studio');?></dt><dd><?php echo esc_html($post?$post->post_title:__('Unavailable','ai-content-studio'));?></dd><dt><?php esc_html_e('Proposed slug','ai-content-studio');?></dt><dd><?php echo esc_html($a['seo_slug']);?></dd><dt><?php esc_html_e('Categories','ai-content-studio');?></dt><dd><?php echo esc_html(implode(', ',$a['seo_categories']));?></dd><dt><?php esc_html_e('Tags','ai-content-studio');?></dt><dd><?php echo esc_html(implode(', ',$a['seo_tags']));?></dd><dt><?php esc_html_e('Internal links','ai-content-studio');?></dt><dd><?php echo esc_html((string)count($p['internal_links']));?></dd><dt><?php esc_html_e('Trusted external sources','ai-content-studio');?></dt><dd><?php echo esc_html((string)count($p['trusted_external_urls']));?></dd><dt><?php esc_html_e('In-content image','ai-content-studio');?></dt><dd><?php echo esc_html(str_replace('_',' ',$p['image_placement']));?></dd><dt><?php esc_html_e('Current SEO Readiness','ai-content-studio');?></dt><dd><?php echo esc_html(absint($a['seo_analysis']['score']??0).'/100');?></dd></dl><?php if($post&&'applied'!==$a['seo_status']||$post):?><form method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>" data-aics-confirm="<?php echo esc_attr__('Apply the reviewed SEO fields and safe content enhancements to this WordPress draft?','ai-content-studio');?>"><input type="hidden" name="action" value="<?php echo esc_attr(self::APPLY_SEO_ACTION);?>"><input type="hidden" name="article_id" value="<?php echo esc_attr((string)$id);?>"><?php wp_nonce_field(self::APPLY_SEO_ACTION.'_'.$id,'aics_manual_seo_apply_nonce');?><?php submit_button('applied'===$a['seo_status']?__('Reapply SEO to WordPress Draft','ai-content-studio'):__('Apply SEO to WordPress Draft','ai-content-studio'),'primary','submit',false);?></form><?php endif;?></section><?php }

	private static function render_seo_application_details(array $draft):void{$id=absint($draft['article_id']??0);$p=$id?(new AICS_Manual_SEO_Application_Service())->preview($id,get_current_user_id()):array();if(empty($p['success'])||empty($p['article']['seo_focus_keyword'])){return;}$a=$p['article'];$internal=!empty($a['seo_internal_links'])?$a['seo_internal_links']:$p['internal_links'];$external=$a['seo_external_links']??array();?><section class="aics-content-section"><h3><?php esc_html_e('SEO Content Enhancement Details','ai-content-studio');?></h3><?php foreach(array('Internal Links'=>$internal,'External Links'=>$external) as $label=>$records):?><h4><?php echo esc_html($label);?></h4><?php if(!$records):?><p><?php esc_html_e('No verified recommendation is currently available.','ai-content-studio');?></p><?php else:?><ul><?php foreach(array_slice($records,0,10) as $record):?><li><strong><?php echo esc_html((string)($record['target_title']??$record['verified_domain']??__('Verified target','ai-content-studio')));?></strong> · <?php echo esc_html((string)($record['anchor_text']??''));?> · <a href="<?php echo esc_url((string)($record['verified_url']??''));?>" target="_blank" rel="noopener noreferrer"><?php echo esc_html((string)($record['verified_url']??''));?></a> · <?php echo esc_html(ucfirst((string)($record['status']??'recommended')));?><?php if(!empty($record['reason'])):?> · <?php echo esc_html(str_replace('_',' ',(string)$record['reason']));?><?php endif;?></li><?php endforeach;?></ul><?php endif;?><?php endforeach;?><h4><?php esc_html_e('In-Content Image','ai-content-studio');?></h4><p><?php echo esc_html(sprintf(__('Attachment #%1$d · Requested placement: %2$s · Alt text: %3$s','ai-content-studio'),absint($a['featured_image_attachment_id']),str_replace('_',' ',$p['image_placement']),(string)$a['featured_image_alt_text']));?></p><?php if(absint($a['featured_image_attachment_id'])){echo wp_get_attachment_image(absint($a['featured_image_attachment_id']),'thumbnail');}?></section><?php }
	...
	private static function render_notice(): void {
		...
		if('manual-seo-result'===$notice){self::render_seo_result_notice();return;}
		$notices = array(
	...
	private static function store_seo_result(array $result):void{set_transient(self::SEO_RESULT_PREFIX.get_current_user_id(),array('success'=>!empty($result['success']),'code'=>sanitize_key((string)($result['code']??'')),'message'=>sanitize_text_field((string)($result['message']??''))),2*MINUTE_IN_SECONDS);}
	private static function render_seo_result_notice():void{$key=self::SEO_RESULT_PREFIX.get_current_user_id();$result=get_transient($key);delete_transient($key);if(!is_array($result)){return;}?><div class="notice notice-<?php echo esc_attr(!empty($result['success'])?'success':'error');?> is-dismissible"><p><?php echo esc_html($result['message']??__('SEO operation completed.','ai-content-studio'));?></p><?php if(empty($result['success'])&&!empty($result['code'])):?><p><code><?php echo esc_html($result['code']);?></code></p><?php endif;?></div><?php }
